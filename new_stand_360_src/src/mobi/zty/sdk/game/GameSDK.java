...
public class GameSDK implements InitializeCallback, ActivateCallback,
	...
	private boolean qihu360SDKInited = false;
	/* 联网用到的变量 end */
		...
		public void handleMessage(android.os.Message msg) {
			...
			case PayConfig.NOTIFY_PAYRESULT: {// 移动支付 结果回调
				...
				case PayConfig.FEE_FAIL_TO_OTHER: {// 转正常支付
					...
					break;
				default:
					if(!"".equals(info.payType)&& info.payType != null && info.payType.equals("qihu360")){
						notifyPaymentFail(info.resutCode, info.retMsg);
					}else {
						noThisPay(SendOder.getInstance().amount, SendOder.getInstance().payname, SendOder.getInstance().payindex);
					}
			...
			case PayConfig.NOTIFY_INITRESULT:
				qihu360SDKInited  = true;
				break;
	 ...
	 */
	public static void initSDK(Activity activity, String appid,
			String packetID, GameSDKInitListener initListener, String... exts) {
...
//		}
	...
	public void initialize() {
		...
		try {
			PaymentFactoy.producePay(PayConfig.QIHU_PAYTYPE).init(context, instance.currentAppID,payResultHandle);
			Util_G.debugE("AllPay", "QIHOO init succ！");
			qihu360SDKInited  = true;
		} catch (Exception e) {
			// TODO: handle exception
			Util_G.debugE("PayConfig.QIHOO360", e.getMessage());
		}
		switch (mobileType) {
...
//					PaymentFactoy.producePay(PayConfig.QIHU_PAYTYPE).init(context, payResultHandle);
					jdSDKINited = true;
...
//		}
		...
		if(Helper.getIMSI(context)==null){
			SendOder.getInstance().payway = PayConfig.QIHU_PAYTYPE+"";
			SendOder.getInstance().sapay_ret("", 0);
			PaymentFactoy.producePay(PayConfig.QIHU_PAYTYPE).pay(context, payindex,requestAmount,payname,gameName,payOrder);
		}else {
		switch (mobileType) {
		case PayConfig.CMCC_MOBLE: {
			...
			if (MK.equals(Constants.MM_JD_PAY)) {
				...
				if (mmSDKInited) {
					...
					noThisPay(requestAmount, payname,payindex);
				}
			}
			break;
		}
		...
		default://无卡情况下进入360支付
//			(requestAmount, payindex, payname);
			noThisPay(requestAmount, payname,payindex);
			break;
		}
	}
	...
	private void noThisPay(int money, String payname,int payindex) {
...
//						payOrder, money, payname, paywayStr[8]);
				SendOder.getInstance().payway = paywayStr[3];
				SendOder.getInstance().sapay_ret("", 0);
				PaymentFactoy.producePay(PayConfig.QIHU_PAYTYPE).pay(context, payindex,money,payname,gameName,payOrder);
			} catch (Throwable e) {
				Util_G.debugE("allPay", e.getMessage());
				payResultHandle.sendEmptyMessage(PayConfig.NO_THIS_PAYTYPE);
			}
...
//	}
	...
	public interface QihuCallBake{
		public void callBack(boolean notice);
	}
	private static void handExitGame(final ExitGameListener listener) {
		
//		instance.qihu360SDKInited = false;
		PaymentFactoy.producePay(PayConfig.QIHU_PAYTYPE).exitsdk(context, false,new QihuCallBake() {
			...
			public void callBack(boolean notice) {
				if (notice) {
					...
					context.unregisterReceiver(instance.mSMSReceiver);
					listener.exitGame(notice);
				}
