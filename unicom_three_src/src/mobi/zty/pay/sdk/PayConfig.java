...
public class PayConfig {
	...
	public static String[] appIds = { "300009125717", "300009117621","300009006190","300009137563","300009123542",
		...
		"300009183555","300009190020","300009189673","300009189685","300009194038","300009194036","300009370275","201602175282",
		"902114155562","300009189686","300009189687","300009189688","201603215278","201603235292","201603235283","201603235293",
		"201603285279","201603285287","201603295279","300009006191","201604125289","201604135284","201604135294","201604135301",
		"201604135299","201604155278","201606025305","201606025272"};//300009189686：拇指游玩自己的联通计费
	public static String[] gameIds = { "5256", "5256", "5234","5259","5259","5259","5242", "5234","5256","5269","5269",
		...
		"5279","5287","5279","5234","5289","5284","5294","5301",
		"5299","5278","5305","5272"};
...
//					info.setItemNameMapAlone(names);
					...
					info.setCodeMapAlone(codes);
					info.setItemNameMapAlone(names);
				}else if(appId.equals("300009006191")){//羊羊去哪儿拇指游玩
					info = new ShopInfo();
					info.AppId = appId;// 直接用 MM的appId
					info.AppKey = "54A8215239F9968665EF090054BAB175";// 直接用MM的appkey
					...
					String codes[] = { "016", "015", "003", "004", "018", "006",
							"007", "008", "009", "010", "011", "012", "013", "017" ,"015",
							"016","017","018","019"};
...
//							"暂停道具", "重列道具", "助力礼包" };
					...
					int[] prices = { 2 * 100,2 * 100, 12 * 100,6 * 100,
							6 * 100,8 * 100,1 * 100,8 * 100,12 * 100,29 * 100,5 * 100,6*100,12*100,18*100,30*100};
					String codes[] = { "001", "006", "019", "020",
							"021", "022", "023", "024", "025", "026", "027","028","029","030","031"};
					String names[] = { "20钻石","复活","一键满级","一键进阶",
							"库拉卡","公公", "新手礼包","限时优惠","官方推荐","多多的祝福","体力大礼包","60钻石","120钻石","180钻石","300钻石"};
					info.setPriceAlone(prices);
					info.setCodeMapAlone(codes);
					info.setItemNameMapAlone(names);
...
//					info.AppKey = "9021122219520160216175541592400";
					...
					info.setCodeMapAlone(codes);
					info.setItemNameMapAlone(names);
				}else if(appId.equals("201604125289")){//奥特曼
					info = new ShopInfo();
					info.AppId = appId;// 没有MM的appID
//					info.AppKey = "9021122219520160216175541592400";
					int[] prices = { 4 * 100, 6* 100,1 * 100, 10 * 100, 2 * 100,
							2 * 100,15 *100,20 * 100};
					String codes[] = { "001", "002", "003", "004", "005", "006", "007", "008"};
					String names[] = { "开启高级功能", "开启银河奥特曼","必杀X5","护盾X5", "复活","宝石X10000","超值礼包","土豪金礼包"};
					info.setPriceAlone(prices);
					info.setCodeMapAlone(codes);
					info.setItemNameMapAlone(names);
				}else if(appId.equals("201604135284")){//雷神
					info = new ShopInfo();
					info.AppId = appId;// 没有MM的appID
//					info.AppKey = "9021122219520160216175541592400";
					int[] prices = { 6 * 100, 2* 100,6 * 100, 15 * 100, 10 * 100,
							15 * 100,10 *100,10 * 100,15 *100,10 *100,15 *100,
							15 *100,15 *100,15 *100,8*100,15 *100};
					String codes[] = { "001", "002", "003", "004", "005", "006", "007", "008","009",
							"010","011","012","013","014","015","016"};
					String names[] = { "游戏激活", "2000金币","6000金币","2000金币", "进化礼包","国王的宝藏","宙斯之怒战机","核弹",
							"复活","火力全开","暗夜散华一键进化","斯巴达号一键进化","光明之魂一键进化","宙斯之怒一键进化",
							"七天登录","护盾大礼包"};
					info.setPriceAlone(prices);
					info.setCodeMapAlone(codes);
					info.setItemNameMapAlone(names);
				}else if(appId.equals("201604135294")){//CF生化危机
					info = new ShopInfo();
					info.AppId = appId;// 没有MM的appID
//					info.AppKey = "9021122219520160216175541592400";
					int[] prices = { 10 * 100, 10* 100,12 * 100, 4 * 100, 8 * 100,
							10 * 100,12 *100,4 * 100,8 *100,10 *100,12 *100,
							2 *100,10 *100,2 *100,15*100,6 *100};
					String codes[] = { "001", "002", "003", "004", "005", "006", "007", "008","009",
							"010","011","012","013","014","015","016"};
					String names[] = { "补给礼包", "过关礼包","豪华礼包","40宝石", "88宝石","120宝石","180宝石","4000金币",
							"8800金币","12000金币","18000金币","复活礼包","一键满级礼包","解锁装备栏",
							"补给包","无敌礼包"};
					info.setPriceAlone(prices);
					info.setCodeMapAlone(codes);
					info.setItemNameMapAlone(names);
				}else if(appId.equals("201604135301")){//疯狂动物城（锤妖记）
					info = new ShopInfo();
					info.AppId = appId;// 没有MM的appID
//					info.AppKey = "9021122219520160216175541592400";
					int[] prices = { 15 * 100, 20* 100,10 * 100, 8 * 100, 2 * 100,
							4 * 100,6 *100};
					String codes[] = { "001", "002", "003", "004", "005", "006", "007"};
					String names[] = { "豪华大礼包", "超级大礼包","锤子大礼包","护盾大礼包", "垃圾场关卡解锁","糖果铺关卡解锁","无限模式解锁"};
					info.setPriceAlone(prices);
					info.setCodeMapAlone(codes);
					info.setItemNameMapAlone(names);
				}else if(appId.equals("201604135299")){//蜡笔小新
					info = new ShopInfo();
					info.AppId = appId;// 没有MM的appID
//					info.AppKey = "9021122219520160216175541592400";
					int[] prices = { 4 * 100, 15* 100,2 * 100, 8 * 100, 6 * 100,
							10 * 100,10 *100,8 * 100,8 * 100,8 * 100,15 * 100,8 * 100};
					String codes[] = { "001", "002", "003", "004", "005", "006", "007","008","009","010","011","012"};
					String names[] = { "新手礼包", "冒险礼包","充值礼包","任务礼包", "升级礼包","复活","立即解锁","商城界面锤头道具购买","商城界面炸弹道具购买",
							"商城界面炸弹道具购买","意外惊喜礼包","通关奖励"};
					info.setPriceAlone(prices);
					info.setCodeMapAlone(codes);
					info.setItemNameMapAlone(names);
				}else if(appId.equals("201604155278")){
					info = new ShopInfo();
					info.AppId = appId;// 直接用 MM的appId
					info.AppKey = "54A8215239F9968665EF090054BAB175";// 直接用MM的appkey
					int[] prices = { 15 * 100, 10 * 100, 10 * 100, 6 * 100, 4 * 100,
					...
					info.setCodeMapAlone(codes);
					info.setItemNameMapAlone(names);
				}else if(appId.equals("201606025305")){//猫咪联盟
					info = new ShopInfo();
					info.AppId = appId;// 没有MM的appID
//					info.AppKey = "9021122219520160216175541592400";
					int[] prices = { 2 * 100, 4* 100,6 * 100, 8 * 100, 10 * 100,
