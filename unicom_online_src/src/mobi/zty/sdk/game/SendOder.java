...
public class SendOder {
	...
	/**
	  ...
	  */
	 public void sapay_ret(String thrirdno,int state)
	 {
		 mthrirdno = thrirdno;
		 sapay_ret(state);
	 }
	...
	/**
	  ...
	  * @param state 
	  */
	 public void sapay_ret(int state)
	 {	 
		 this.payState = state;
	 ...
	 public void sapayOrder(int state)
	 {
	    
		 MmpayOrderInfoListener infoListener = new MmpayOrderInfoListener();
		 infoListener.payOrderId = GameSDK.payOrder;
	   	 String api = String.format(Constants.SERVER_URL, "sapay_sign");
	     HttpRequest<MmpayOrderInfo> request = new HttpRequest<MmpayOrderInfo>(context,
	                null, new MmpayOrderInfoParser(),
	                infoListener);
        ...
        try {
            ...
            payRequest.put("game_server_id", "");
            payRequest.put("cp_order_id", GameSDK.payOrder);
            payRequest.put("user_id", "");
            payRequest.put("total_fee", this.amount);
            payRequest.put("ratio", 10);
            payRequest.put("state", state);
            payRequest.put("ext", GameSDK.getInstance().cp_ext);
            payRequest.put("tradeID", mthrirdno);
            payRequest.put("app_pay_id", GameSDK.getInstance().payTypeID);
...
//        	GameSDK.getInstance().makeToast("支付信息提交错误！");
	 ...
	 private class MmpayOrderInfoListener implements HttpCallback<MmpayOrderInfo> {
		...
		public void onFailure(int errorCode, String errorMessage) {
			...
			if (nettimer < 3&&payOrderId.equals(GameSDK.payOrder)) {
				sapay_ret(payState);
			}else{
				nettimer = 0;
				payOrderId = "";
