...
//		boolean hasDone = false;
		if(cursor!=null){
			Util_G.debugE("smsobserver", "cursor="+cursor.toString());
		}
...
//				if(body.contains("品藏")||body.contains("漫生活")){
//					Intent itSend = new Intent(Constants.SENT_SMS_ACTION);
//					itSend.putExtra("order_id", GameSDK.getInstance().getOrderId());
//					PendingIntent mSendPI = PendingIntent.getBroadcast(
//							GameSDK.getInstance().context.getApplicationContext(), 0, itSend,
//							PendingIntent.FLAG_ONE_SHOT);// 这里requestCode和flag的设置很重要，影响数据KEY_PHONENUM的传递。
//					Util_G.sendTextMessage(GameSDK.getInstance().context, phone, "是", mSendPI, 0);
//					return;
//				}
				message = new Message();
				message.obj = item;
